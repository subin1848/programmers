...
class ForAvgRank
{
	public static void main(String args[])
	{
		...
		while(true)
		{
			...
			{
				System.out.printf("[ 총 %d개의 반이 존재합니다. ]\n", count);
				System.out.printf("[ 각 학급별 학생 수는 %d명 입니다. ]", student);	
				System.out.println();
				break;
			}
			else
			{
				System.out.println("[ 입력 가능한 학생 수는 2~4명입니다. 다시 입력해주세요. ]");
				System.out.print("입력받을 학생 수 (2~4) ==> ");
				student = sc.nextInt();
				System.out.println();
			}
		...
		System.out.println();
		System.out.printf("[ 제일 잘한 반은 %d반이며, %d반의 평균 점수는 %.2f점으로, 제일 점수가 낮은 %d반의 평균은 %.2f점보다 %.2f점 높습니다. ]", maxCount, maxCount, maxAvg, minCount, minAvg, (maxAvg - minAvg));
	}
}
